        ...
        <CardContent>
          {selectedType && (
            <Box>
              ...
              >
                ...
                >
                  <Autocomplete
                    ...
                    value={department}
                    onChange={(e, newValue) => setDepartment(newValue)}
                    sx={{ width: { xs: "100%", sm: 300, md: 200 } }}
                    renderInput={(params) => <TextField {...params} label="Department" size="small" variant="outlined" />}
                  />
                  <TextField
                    size="small"
                    label="Search"
              ...
              {isMobile ? (
                <Box>
                  {filteredTickets.length > 0 ? (
                      ...
                      .map((t) => (
                        <Card sx={{ mb: 2, borderRadius: 2 }} key={t.id}>
                          <CardContent>
                            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                                <Typography fontWeight={700} color="#667eea">#{t.ticket_no} -</Typography>
                                <Chip
                                  label={t.priority_detail?.field_values || "-"}
                                  size="small"
                                  sx={{
                            ...
                            <Typography fontWeight={600} mt={1} color="#2D3748">{t.title}</Typography>
                            <Tooltip title={t.description || "No description"}>
                              <Typography sx={{ maxWidth: 200, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: "text.secondary", mt: 0.5 }}>
                                {t.description || "-"}
                              </Typography>
                            </Tooltip>
                            <Typography fontSize={13} mt={1.5}>
                              <strong style={{ color: "#4A5568" }}>Category:</strong> {t.category_detail?.category_name || "-"} / {t.subcategory_detail?.subcategory_name || "-"}
                            </Typography>
                            <Typography fontSize={13} mt={1}>
                              <strong style={{ color: "#4A5568" }}>Dept | Loc:</strong> {t.department_detail?.field_name || "-"} | {t.location_detail?.field_name || "-"}
                            </Typography>
                            <Typography fontSize={12} color="#718096" mt={1.5}>
                              Open: {new Date(t.created_date).toLocaleDateString()} <br />
                              Update: {new Date(t.updated_date).toLocaleDateString()}
                            ...
                            <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 2, gap: 1 }}>
                              <Tooltip title="Follow-up Chat">
                                {/* ← FIXED: Pass the full ticket object */}
                                <IconButton onClick={() => handleChatDrawerOpen(t)} size="small" sx={{ color: "#667eea" }}>
                                  <ChatIcon />
                                </IconButton>
                              </Tooltip>
                              <Tooltip title="View Details">
                                <IconButton onClick={() => handleTicketClick(t.ticket_no)} sx={{ color: "#667eea" }} size="small">
                                  <VisibilityIcon />
                                </IconButton>
                              </Tooltip>
                            </Box>
                    ...
                    <Typography align="center" py={4} color="#718096">No tickets found.</Typography>
                  )}
                </Box>
              ) : (
                <Card sx={{ borderRadius: 3, boxShadow: 2, overflow: "hidden" }}>
                      ...
                      <TableHead>
                        <TableRow sx={{ backgroundColor: "#F7FAFC" }}>
                          {[
                            ...
                            <TableCell key={i} sx={{ fontWeight: 700, whiteSpace: "nowrap", color: "#2D3748", py: 2 }}>
                              {title}
                            </TableCell>
                          ))}
                        </TableRow>
                      ...
                      <TableBody>
                        {filteredTickets.length > 0 ? (
                          filteredTickets
                            ...
                            .map((t) => (
                              <TableRow key={t.id} hover>
                                <TableCell sx={{ color: "#667eea", fontWeight: 600 }}>#{t.ticket_no}</TableCell>
                                <TableCell>
                                  <Tooltip title={t.title}>
                                    <Typography sx={{ maxWidth: 200, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.title}</Typography>
                                  </Tooltip>
                                </TableCell>
                                <TableCell>
                                  <Tooltip title={t.description || "No description"}>
                                    <Typography sx={{ maxWidth: 200, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.description || "-"}</Typography>
                                  </Tooltip>
                                </TableCell>
                                <TableCell>
                                  <Typography fontWeight={500}>{t.status_detail?.field_values}</Typography>
                                  <Typography fontSize="0.85rem">{t.priority_detail?.field_values}</Typography>
                                </TableCell>
                                <TableCell>
                                  <Typography fontWeight={500}>{t.category_detail?.category_name || "-"}</Typography>
                                  <Typography fontSize="0.85rem">{t.subcategory_detail?.subcategory_name || "-"}</Typography>
                                </TableCell>
                                <TableCell>
                                  <Typography fontWeight={500}>{t.department_detail?.field_name}</Typography>
                                  <Typography fontSize="0.85rem">{t.location_detail?.field_name}</Typography>
                                </TableCell>
                                <TableCell>{t.requested_detail?.email}</TableCell>
                                <TableCell>
                                  <Typography fontSize="0.9rem">{new Date(t.created_date).toLocaleDateString()}</Typography>
                                  <Typography fontSize="0.8rem">{new Date(t.updated_date).toLocaleDateString()}</Typography>
                                </TableCell>
                                <TableCell>
                                  <Box sx={{ display: "flex", gap: 1 }}>
                                    <Tooltip title="Follow-up Chat">
                                    ...
                                    <Tooltip title="View Details">
                                      <IconButton onClick={() => handleTicketClick(t.ticket_no)} size="small" sx={{ color: "#667eea" }}>
                                        <VisibilityIcon />
                                      </IconButton>
                                    </Tooltip>
                                  </Box>
                          ...
                          <TableRow>
                            <TableCell colSpan={9} align="center" sx={{ py: 4, color: "#718096" }}>No tickets found.</TableCell>
                          </TableRow>
                        )}
                      </TableBody>
                    </Table>
              ...
              {filteredTickets.length > 0 && (
                <Stack direction={isMobile ? "column" : "row"} justifyContent="space-between" alignItems="center" sx={{ py: 2 }}>
...
// import { useNavigate } from "react-router-dom";
// import { toast } from "react-toastify";
// import { fetchAdminMessages, sendMessage, fetchAdminTickets } from "../../Api";
