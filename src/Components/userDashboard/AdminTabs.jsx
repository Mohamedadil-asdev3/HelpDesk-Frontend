...
import { useEffect, useMemo, useState } from "react";
import { useTheme } from "@mui/material/styles";
import { Box, Card, CardContent, Typography, Grid, TextField, Button, Table, TableHead, TableRow, TableCell, TableBody, TableContainer, useMediaQuery, Autocomplete, Stack, CircularProgress, IconButton, Drawer, Divider, Chip, Avatar, Icon, Tooltip, Pagination } from "@mui/material";
import NewReleasesIcon from '@mui/icons-material/NewReleases';
import DoneAllIcon from '@mui/icons-material/DoneAll';
import LockIcon from '@mui/icons-material/Lock';
...
const AdminTabs = () => {
    ...
    const [isProtected, setIsProtected] = useState(false); // Shield toggle on/off
    ...
    const statusCards = [
        ...
        { id: "clarification_applied", label: "Clar. Supplied", color: "error", icon: <CancelIcon />, count: adminData?.admin_stats?.clarification_applied || 0 },
        { id: "clarification_required", label: "Clar. Required", color: "error", icon: <CancelIcon />, count: adminData?.admin_stats?.clarification_required || 0 },
    ];
    ...
    //     localStorage.setItem('selectedTicketId', ticketNo);
    //     console.log('Navigating to Approval');
    //     navigate('/Approval', { state: { editMode: false } });
    // };

    const handleTicketClick = (ticketNo) => {
        localStorage.setItem('selectedTicketId', ticketNo);
        navigate(`/Approval?mode=view`);
    };

    const handleTicketEdit = (ticketNo) => {
        localStorage.setItem('selectedTicketId', ticketNo);
        navigate(`/Approval?mode=edit`);
    }
    ...
    const headingMap = {
        ...
        clarification_applied: "CLARIFICATION SUPPLIED",
                                                                ...
                                                                <TableCell>
                                                                    <Box sx={{ display: "flex" }}>
                                                                        <Tooltip title="Edit Ticket">
                                                                            <IconButton
                                                                                onClick={() => handleTicketEdit(t.ticket_no)}
                                                                                color="warning"
                                                                                size="small"
                                                                            >
                                                                                <EditIcon />
            ...
            <Drawer anchor="right" open={showFollowUpChat} onClose={() => setShowFollowUpChat(false)}>
                <Box sx={{ width: { xs: "100%", sm: 500 }, height: "100%", display: "flex", flexDirection: "column" }}>
                            ...
                            groupedChats.map(g => (
                                <Box key={g.date} sx={{ mb: 4 }}>
                                    ...
                                    {g.messages.map(m => {
                                        ...
                                        const isProtected = m.protected === true;
                                        const messageId = m.id;
                                        const isRevealed = revealedMessages.has(messageId);

                                        // Only sender or receiver can see decrypted message
                                        //const canViewDecrypted = isMe || Number(m.receiver) === Number(currentUserId);
                                        const canViewDecrypted = Number(m.sender) === Number(currentUserId) || Number(m.receiver) === Number(currentUserId);

                                        // Display logic
                                        let displayText = m.message;
                                        if (isProtected && canViewDecrypted && isRevealed && m.decrypted_message) {
                                            displayText = m.decrypted_message;
                                        }
                                            ...
                                            >
                                                <Box
                                                    sx={{
                                                        ...
                                                        borderRadius: 18,
                                                        boxShadow: 1,
                                                        border: isProtected ? "2px solid #4CAF50" : "none",
                                                        position: "relative",
                                                    }}
                                                >
                                                    {/* Shield Badge */}
                    ...
                    <Box sx={{ p: 2, borderTop: 1, borderColor: "divider", bgcolor: "background.paper" }}>
                        ...
                        {/* Indicator when protected mode is on */}
                        {isProtected && (
