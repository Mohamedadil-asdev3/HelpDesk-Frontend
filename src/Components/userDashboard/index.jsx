...
const UserDashboard = () => {
  ...
  const roleName = roleMapping?.role_name;
  const isTechnician = roleName === "Technician";
  const isAdmin = roleName === "Admin";
  ...
  const fetchDashboard = async (filters = {}) => {
    try {
      ...
      // 🔹 Technician → Approver tickets only
      if (isTechnician) {
        res = await fetchApproverTickets(filters);
      } else {
        // 🔹 Admin / User → Full dashboard counts
        res = await ticketcounts(filters);
      }
  ...
  --------------------------*/
  const userStatus = isTechnician
    ? dashboardData
    : dashboardData?.user_stats || null;
