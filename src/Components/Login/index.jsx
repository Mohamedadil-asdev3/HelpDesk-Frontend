...
// import { loginAPI, forgotPasswordAPI, changePasswordAPI, fetchEntitiesAPI } from "../../Api"; // Assuming API functions are in api.js; adjust path as needed
// const Login = () => {
...
//                 setShowChangePassword(true); // Show change password modal
//                 // Temporarily store role data for later
//                 localStorage.setItem("temp_role_data", JSON.stringify(data.user));
//                 // Do not proceed to role selection yet
//             } else {
...
//                 // No entities but has roles: fetch entities and show entity selection modal
//                 await fetchEntitiesForModal();
//                 setIsEntityOnlyModal(true);
//                 setModalOpen(true);
//                 // Store mappings as empty, will handle in modal
//                 setRoleMappings([]);
//                 return;
//             } else {
//                 // No entities or roles, default
//                 localStorage.setItem("selected_role_mapping", JSON.stringify({ role_name: 'user' }));
//                 navigate("/UserDashboard");
//                 return;
//             }
//         }
...
//         }
//     };
//     const handleEntityOnlySelection = (selectedEnt) => {
//         const userDataStr = localStorage.getItem("user_data") || "{}";
//         const userData = JSON.parse(userDataStr);
//         const hasSuperAdmin = userData.roles?.some(role => role.name.toLowerCase() === 'superadmin' || role.name.toLowerCase() === 'super admin');
//         let roleName = hasSuperAdmin ? 'superadmin' : (userData.roles?.[0]?.name || 'user');
//         let roleId = null;
//         if (roleName !== 'superadmin') {
//             const roleObj = userData.roles?.find(r => r.name.toLowerCase() === roleName);
//             roleId = roleObj?.id;
//         }
...
//             role_id: roleId,
//             role_name: roleName,
//             ...(roleName === 'superadmin' && { all_access: true })
//         };
...
//         } catch (err) {
//             setError({ message: err?.response?.data?.message || "Failed to change password. Try again.", username: "", password: "" });
//         } finally {
...
//             setForgotEmail("");
//             setError({ message: "Password reset link sent!", username: "", password: "" });
//         } catch (err) {
...
//                 </CardContent>
//             </Card>
//             {/* Role Mapping Modal - Conditional based on isEntityOnlyModal */}
//             <Dialog open={modalOpen} fullWidth PaperProps={{ sx: { width: 400 } }}>
//                 <DialogTitle>{isEntityOnlyModal ? "Select Entity" : "Select Entity and Role"}</DialogTitle>
...
//                                         <ListItemButton
//                                             key={ent.id}
//                                             onClick={() => handleEntityOnlySelection(ent)}
//                                             sx={{ py: 1 }}
//                                         >
//                                             <ListItemIcon>
//                                                 <BusinessIcon />
